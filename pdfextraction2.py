...
for file in care_files:
  df_pdf = tabula.read_pdf(file, pages='all')
  new_header = df_pdf[0].iloc[0]
  df_pdf[0] = df_pdf[0][4:] 
  df_pdf[0].columns = new_header 
  df_filtered=df_pdf[0]
  df_filtered.columns = ['Series','ISIN Code','Reference Asset', 'Issue Date*', 'Maturity Date','Face Value per Debenture', 'Valuation']
  print(df_filtered)
  care_df_list.append(df_filtered)
